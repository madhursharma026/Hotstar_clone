...
@views.route("/homepage_categories/<UserMobileNumber>/<UserLoginToken>", methods=['GET'])
def homepage_categories(UserMobileNumber, UserLoginToken):
    try:
        conn = mysql.connector.connect(host="localhost", user="root", password="", database="admin_buddy9_qa")
        ...
        my_cursor.execute("SELECT * FROM members WHERE mobile = " + str(UserMobileNumber))
        UserLoginInSingleDeviceOutput = my_cursor.fetchone()
        if UserLoginInSingleDeviceOutput != None:
            if UserLoginInSingleDeviceOutput[3] == str(UserLoginToken):
...
@views.route("/search_word/<SearchWord>/<UserMobileNumber>/<UserLoginToken>", methods=['GET'])
def search_word(SearchWord, UserMobileNumber, UserLoginToken):
    all_search_data = []
    conn = mysql.connector.connect(host="localhost", user="root", password="", database="admin_buddy9_qa")
    my_cursor = conn.cursor(buffered=True)
    my_cursor.execute("SELECT * FROM members WHERE mobile = " + str(UserMobileNumber))
...
@views.route("/single_videos/<movie_id>/<UserMobileNumber>/<UserLoginToken>", methods=['GET'])
def single_videos(movie_id, UserMobileNumber, UserLoginToken):
    all_data = []
    conn = mysql.connector.connect(host="localhost", user="root", password="", database="admin_buddy9_qa")
    ...
    if UserLoginInSingleDeviceOutput != None:
        if UserLoginInSingleDeviceOutput[3] == str(UserLoginToken):
            my_cursor.execute("SELECT * FROM files Where status = 1 AND movie_id = " + movie_id)
            movie_src_details = my_cursor.fetchone()
            my_cursor.execute(
...
@views.route("/movie_category_videos/<movie_category_name>/limit=<limit>/<UserMobileNumber>/<UserLoginToken>", methods=['GET'])
def movie_category_videos(movie_category_name, limit, UserMobileNumber, UserLoginToken):
    all_data = []
    conn = mysql.connector.connect(host="localhost", user="root", password="", database="admin_buddy9_qa")
    my_cursor = conn.cursor()
    my_cursor.execute("SELECT * FROM members WHERE mobile = " + str(UserMobileNumber))
    ...
    if UserLoginInSingleDeviceOutput != None:
        if UserLoginInSingleDeviceOutput[3] == str(UserLoginToken):
            ...
            # sql_select_query = "Select * from movie where cat_id = %s limit " + str(limit)
...
@views.route("/single_category_videos/<category_id>/limit=<limit>/<UserMobileNumber>/<UserLoginToken>", methods=['GET'])
def single_category_videos(category_id, limit, UserMobileNumber, UserLoginToken):
    all_data = []
    conn = mysql.connector.connect(host="localhost", user="root", password="", database="admin_buddy9_qa")
    my_cursor = conn.cursor()
    my_cursor.execute("SELECT * FROM members WHERE mobile = " + str(UserMobileNumber))
...
@views.route("/single_videos_for_player/<movie_id>/<UserMobileNumber>/<UserLoginToken>", methods=['GET'])
def single_videos_for_player(movie_id, UserMobileNumber, UserLoginToken):
    all_data = []
    movie_src_data = []
    conn = mysql.connector.connect(host="localhost", user="root", password="", database="admin_buddy9_qa")
    my_cursor = conn.cursor()
    ...
    if UserLoginInSingleDeviceOutput != None:
        if UserLoginInSingleDeviceOutput[3] == str(UserLoginToken):
            my_cursor.execute("SELECT * FROM files Where status = 1 AND movie_id = " + movie_id)
            movie_src_details = my_cursor.fetchall()
            for movie_src_details in movie_src_details:
                movie_src_data_in_dict = ("https://" + movie_src_details[9] + "/" + movie_src_details[2] + "/master.m3u8")
                movie_src_data.append(movie_src_data_in_dict)
            my_cursor.execute(
                "SELECT * FROM movie Where status = 1 AND id = " + movie_id)
            movie_details = my_cursor.fetchone()
            data_in_dict = {
